# Local Router Service Installer
# Installs the local-router as a Windows service for the current user

# Get the current directory
$scriptDir = Split-Path -Parent $MyInvocation.MyCommand.Path
$exePath = Join-Path $scriptDir "local-router.exe"

# Check if the executable exists
if (!(Test-Path $exePath)) {
    Write-Host "Error: local-router.exe not found at $exePath" -ForegroundColor Red
    Write-Host "Please build the project first with: go build -o local-router.exe ./server" -ForegroundColor Yellow
    exit 1
}

# Create the service
$serviceName = "local-router"
...
try {
    ...
    nssm set $serviceName Description "Local Router Service for user $username"
